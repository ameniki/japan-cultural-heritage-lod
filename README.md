...
今回の作業では、生成スクリプトの書き方やSPARQLクエリの構文の確認、検証スクリプトの作成といった作業の一部にClaudeを利用しました。題材の選定とスキーマの設計、リンク先とするWikidataエンティティの判断は自分で決めました。各遺産のQID・世界遺産ID・登録年はWikidataのAPIから取得した値と照合し、Turtleの構文と件数、公開したURLの応答は検証スクリプトで確かめました。
